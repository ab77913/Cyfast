...
export default function ProductFixPanel({ projectId, run, defects = [] }) {
  ...
  async function startVerification(fix) {
    setLoading(true);
    setError("");
    try {
        ...
        {fixes.map((fix) => (
          <article className="cyfast-mini-card" key={fix.execution_product_fix_id}>
            ...
            <div className="cyfast-actions">
              ...
              {fix.deployment_status === "DEPLOYED" && !fix.verification_execution_run_id ? <button type="button" className="cyfast-link-button" onClick={() => startVerification(fix)} disabled={loading}>Start verification rerun</button> : null}
            </div>
          </article>
        ))}
      </div>
