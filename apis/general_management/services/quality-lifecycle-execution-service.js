...
async function startLifecycleExecution(lifecycleId, input, actor) {
  ...
  const target = await db.ExecutionTarget.findOne({
    where: {
  ...
  await synchronizeRun(run, actor);
...
async function synchronizeRun(run, actor) {
  ...
  if (lifecycle.status === quality.STATES.FAILED && !["CANCELLED"].includes(runValue.status)) {
