...
def patch_general_management_index(text: str) -> str:
    ...
    registrations = "\n".join(
        [
            "fastify.register(qualityLifecycleContentRoutes);",
            "fastify.register(qualityGenerationRoutes);",
            "fastify.register(executionProductFixRoutes);",
            "fastify.register(executionProductVerificationRoutes);",
        ]
    )
...
'''
    ...
    readiness_binding = '''  const selectedPlatform = String(lifecycle.generation_policy?.selected_platform || lifecycle.generation_policy?.platform || "").toUpperCase();
  const bindingTypes = {
...
'''
    if readiness_binding.strip() not in text:
        marker = "  if (!lifecycle.traceability_complete) errors.push(\"Traceability is incomplete\");"
...
def add_recording_hooks_to_class(text: str, class_name: str, platform: str) -> str:
    ...
    if not before or not after:
        raise ValueError(f"{class_name} must define before_execution and after_execution hooks")
    block = re.sub(
        r"(?m)^(\s+)(async\s+)?def\s+before_execution\s*\(",
        lambda match: f"{match.group(1)}{match.group(2) or ''}def _cyfast_original_before_execution(",
        block,
        count=1,
    )
    block = re.sub(
        r"(?m)^(\s+)(async\s+)?def\s+after_execution\s*\(",
        lambda match: f"{match.group(1)}{match.group(2) or ''}def _cyfast_original_after_execution(",
        block,
        count=1,
    )
    indent = before.group(1)
    async_before = bool(before.group(2))
    async_after = bool(after.group(2))
    before_call = "await self._cyfast_original_before_execution(*args, **kwargs)" if async_before else "self._cyfast_original_before_execution(*args, **kwargs)"
    after_call = "await self._cyfast_original_after_execution(*args, **kwargs)" if async_after else "self._cyfast_original_after_execution(*args, **kwargs)"
    methods = f'''\n{indent}{'async ' if async_before else ''}def before_execution(self, *args, **kwargs):
{indent}    result = {before_call}
{indent}    request = args[0] if args else kwargs.get("request")
{indent}    workspace = args[1] if len(args) > 1 else kwargs.get("workspace")
{indent}    start_desktop_recording(request, workspace, "{platform}")
{indent}    return result

{indent}{'async ' if async_after else ''}def after_execution(self, *args, **kwargs):
{indent}    request = args[0] if args else kwargs.get("request")
{indent}    recording_error = None
{indent}    try:
{indent}        stop_desktop_recording(request)
{indent}    except Exception as error:
{indent}        recording_error = error
{indent}    result = {after_call}
{indent}    if recording_error is not None:
{indent}        raise recording_error
{indent}    return result
'''
