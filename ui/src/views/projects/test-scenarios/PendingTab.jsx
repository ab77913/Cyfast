...
import {
  bulkApproveGeneratedTestScenarios,
  bulkDiscardGeneratedTestScenarios,
  bulkRejectGeneratedTestScenarios,
  discardPendingTestScenarioJobs,
  getTestScenarioGenerationJob,
  listPendingGeneratedTestScenarios,
  regeneratePendingTestScenariosWithAi
...
/**
 * Pending rows can include LLM-shaped JSON (e.g. expected_results as { expected: "..." }
 * or test_steps as an array of objects). React cannot render bare objects — normalize to text.
 */
function scenarioFieldToText(value) {
    ...
    return value
      .map((item) => {
        if (item != null && typeof item === 'object') {
          if (typeof item.expected === 'string') return item.expected;
          try {
...
export default function PendingTab({ project, onApproved }) {
  ...
  const validatedRow = useMemo(() => {
    ...
    return rows.find((x) => x.generated_test_scenario_id === validatorRowId) || null;
  }, [validatorRowId, rows]);
  ...
  const reqNo = (row) =>
    row.requirement?.requirement_no ?? row.requirement_id ?? '';

  const scenarioValFragment = useMemo(() => {
    if (!validatedRow) return null;
    const r = validatedRow;
    const req = r.requirement;
    const scenario_drafts = [
      {
        ...
        automation_possibility_score: r.automation_possibility_score
      }
    ];
    const source_requirement = req
      ? {
      ...
      : r.requirement_id != null
        ? { requirement_id: r.requirement_id, requirement_no: reqNo(r) }
        : null;
    const related_scenario_drafts = rows
      .filter((x) => x.generated_test_scenario_id !== r.generated_test_scenario_id)
      .slice(0, 30)
      .map((x) => ({
        scenario_type: x.scenario_type,
        title: x.title,
        requirement_no: reqNo(x)
      }));
    return {
      scenario_drafts,
      ...(source_requirement ? { source_requirement } : {}),
      related_scenario_drafts
    };
  }, [validatedRow, rows]);
  ...
  const selectedDraftsResolved = useMemo(
    () =>
      selectedIdsArray
        .map((id) => rows.find((r) => r.generated_test_scenario_id === id))
        .filter(Boolean),
  ...
  useEffect(() => {
    const valid = new Set(rows.map((r) => r.generated_test_scenario_id));
  ...
  const fetchPending = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listPendingGeneratedTestScenarios(project.project_id, {
        page: listPage,
        size: pageSize
      });
      const body = res.data || {};
  ...
  const selectAllVisible = () => {
    setSelectedIds(new Set(rows.map((r) => r.generated_test_scenario_id)));
  };
  ...
  const pollJobUntilTerminal = async (jobId) => {
    ...
    throw new Error(
      intl.formatMessage({
        id: 'scenarios-pending-regen-timeout',
        defaultMessage:
  ...
  const submitAiRegeneration = async () => {
    ...
    if (!fb) {
      setError(
        intl.formatMessage({
          id: 'scenarios-pending-regen-feedback-required',
    ...
    setError(null);
    try {
      const res = await regeneratePendingTestScenariosWithAi({
  ...
  const approveByIds = async (ids) => {
    ...
    try {
      const res = await bulkApproveGeneratedTestScenarios({
        project_id: project.project_id,
        candidate_ids: ids
      });
      ...
      if (d.failed > 0) {
        setError(
          intl.formatMessage(
            {
              id: 'scenarios-pending-bulk-approve-partial',
  ...
  const submitReject = async () => {
    ...
    try {
      const res = await bulkRejectGeneratedTestScenarios({
        project_id: project.project_id,
      ...
      if (d.failed > 0) {
        setError(
          intl.formatMessage(
            {
              id: 'scenarios-pending-bulk-reject-partial',
  ...
  const submitBulkDiscardCandidates = async () => {
    ...
    setBulkWorking(true);
    try {
      const res = await bulkDiscardGeneratedTestScenarios({
        project_id: project.project_id,
        candidate_ids: selectedIdsArray
      });
      ...
      if (d.failed > 0) {
        setError(
          intl.formatMessage(
            {
              id: 'scenarios-pending-bulk-discard-partial',
  ...
  const submitDiscardJob = async () => {
    ...
    setBulkWorking(true);
    try {
      const res = await discardPendingTestScenarioJobs({
        project_id: project.project_id,
        job_ids: [jid]
      });
      if (res.status !== 200) throw new Error(res.data?.error || 'Discard failed');
  ...
  const autoScoreCell = (row) =>
    row.automation_possibility_score != null ? String(row.automation_possibility_score) : '—';
    ...
    <div className="py-3">
      <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <p className="text-muted mb-0">
          <FormattedMessage
            id="scenarios-pending-intro"
            defaultMessage="Approve to promote drafts into persisted test scenarios. Reject/discards skip promotion. Regenerate reruns AI on pending rows from one batch."
          />
        </p>
        <Button variant="outline-primary" size="sm" onClick={fetchPending} disabled={loading}>
          <FormattedMessage id="scenarios-pending-refresh" defaultMessage="Refresh" />
        </Button>
      </div>
      ...
      {pendingJobIds.length > 0 && (
        <div className="mb-3">
          <span className="small text-muted me-2">
            <FormattedMessage
              id="scenarios-pending-job-batch-label"
          ...
          {pendingJobIds.map((jid) => (
            ...
            >
              <FormattedMessage id="scenarios-discard-job-pending" defaultMessage="Discard pending — job {id}" values={{ id: jid }} />
            </Button>
          ))}
        </div>
      )}
      ...
      {selectedIdsArray.length > 0 && (
        <Alert variant="info" className="py-2 d-flex flex-wrap align-items-center gap-2 mb-3">
          <span className="small">
            <FormattedMessage id="scenarios-pending-selected-count" defaultMessage="{n} selected" values={{ n: selectedIdsArray.length }} />
          </span>
          <Button variant="success" size="sm" disabled={anyBusy || !project?.project_id} onClick={() => approveByIds(selectedIdsArray)}>
            <FormattedMessage id="scenarios-pending-bulk-approve" defaultMessage="Approve selected" />
          </Button>
          <Button variant="outline-danger" size="sm" disabled={anyBusy || !project?.project_id} onClick={openBulkReject}>
            <FormattedMessage id="scenarios-pending-bulk-reject" defaultMessage="Reject selected" />
          </Button>
          <Button
          ...
          >
            <FormattedMessage id="scenarios-pending-regen-ai" defaultMessage="Regenerate with AI" />
          </Button>
          <Button variant="outline-warning" size="sm" disabled={anyBusy || !project?.project_id} onClick={() => setBulkDiscardModal(true)}>
            <FormattedMessage id="scenarios-pending-bulk-discard" defaultMessage="Discard selected" />
          </Button>
          <Button variant="link" size="sm" className="py-0" disabled={anyBusy} onClick={clearSelection}>
            <FormattedMessage id="scenarios-pending-clear-selection" defaultMessage="Clear selection" />
          </Button>
          {regenerateBarrier.kind === 'multi_job' ? (
            <div className="w-100 small text-warning mb-0">
              <FormattedMessage
                id="scenarios-pending-regen-multi-job"
            ...
            <div className="w-100 small text-warning mb-0">
              <FormattedMessage
                id="scenarios-pending-regen-not-visible"
      ...
      {loading ? (
        ...
        <Alert variant="light" className="border">
          <FormattedMessage id="scenarios-pending-empty" defaultMessage="No scenario drafts awaiting approval." />
        </Alert>
      ) : (
        <>
          {rows.length === 0 ? (
            <Alert variant="light" className="border mb-3">
              <FormattedMessage id="scenarios-pending-page-empty" defaultMessage="No drafts on this page — paginate or refresh." />
            </Alert>
          ) : (
              ...
              <Table responsive hover className="align-middle mb-0">
                <thead className="thead-light">
                  <tr>
                    ...
                    <th>
                      <FormattedMessage id="scenarios-pending-col-type" defaultMessage="Scenario type" />
                    </th>
                    <th>
                      <FormattedMessage id="scenarios-pending-col-req" defaultMessage="Req #" />
                    </th>
                    <th>
                      <FormattedMessage id="scenarios-pending-col-title" defaultMessage="Title" />
                    </th>
                    <th>
                      <FormattedMessage id="scenarios-pending-col-preconditions" defaultMessage="Preconditions" />
                    </th>
                    <th>
                      <FormattedMessage id="scenarios-pending-col-test-steps" defaultMessage="Test Steps" />
                    </th>
                    <th>
                      <FormattedMessage id="scenarios-pending-col-test-data" defaultMessage="Test Data" />
                    </th>
                    <th>
                      <FormattedMessage id="scenarios-pending-col-expected-results" defaultMessage="Expected Results" />
                    </th>
                    <th>
                      <FormattedMessage id="scenarios-pending-col-auto" defaultMessage="Auto %" />
                    </th>
                    <th className="text-center">
                      <FormattedMessage id="scenarios-pending-col-actions" defaultMessage="Actions" />
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.generated_test_scenario_id}>
                      <td className="text-center align-middle">
                        <Form.Check
                          aria-label="select row"
                          checked={selectedIds.has(r.generated_test_scenario_id)}
                          onChange={() => toggleSelected(r.generated_test_scenario_id)}
                          disabled={anyBusy}
                        />
                      </td>
                      <td className="small">{r.scenario_type}</td>
                      <td className="small">{reqNo(r)}</td>
                      <td className="small">{scenarioFieldToText(r.title)}</td>
                      <td className="small text-break" style={{ whiteSpace: 'pre-wrap' }}>
                        {scenarioFieldToText(r.preconditions)}
                      </td>
                      <td className="small text-break" style={{ whiteSpace: 'pre-wrap' }}>
                        {scenarioFieldToText(r.test_steps)}
                      </td>
                      <td className="small text-break" style={{ whiteSpace: 'pre-wrap' }}>
                        {scenarioFieldToText(r.test_data)}
                      </td>
                      <td className="small text-break" style={{ whiteSpace: 'pre-wrap' }}>
                        {scenarioFieldToText(r.expected_results)}
                      </td>
                      <td className="small text-center">{autoScoreCell(r)}</td>
                      <td className="text-center text-nowrap">
                        <Button
                          variant="outline-info"
                          size="sm"
                          className="me-1"
                          disabled={actionBusy === r.generated_test_scenario_id || !project?.project_id || bulkWorking}
                          onClick={() => setValidatorRowId(r.generated_test_scenario_id)}
                        >
                          <FormattedMessage id="scenarios-pending-validator" defaultMessage="Validate" />
                        </Button>
                        <Button
                          variant="success"
                          size="sm"
                          className="me-1"
                          disabled={actionBusy === r.generated_test_scenario_id || bulkWorking}
                          onClick={() => approveByIds([r.generated_test_scenario_id])}
                        >
                          <FormattedMessage id="scenarios-pending-approve" defaultMessage="Approve" />
                        </Button>
                        <Button
                          variant="outline-danger"
                          size="sm"
                          disabled={actionBusy === r.generated_test_scenario_id || bulkWorking}
                          onClick={() => openReject(r.generated_test_scenario_id)}
                        >
                          <FormattedMessage id="scenarios-pending-reject" defaultMessage="Reject" />
                        </Button>
                      </td>
                    </tr>
                  ))}
      ...
      <Modal show={regenerateModalOpen} onHide={() => !bulkWorking && setRegenerateModalOpen(false)} centered>
        <Modal.Header closeButton={!bulkWorking}>
          <Modal.Title>
            <FormattedMessage id="scenarios-pending-regen-modal-title" defaultMessage="Regenerate drafts with AI" />
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group className="mb-3">
            <Form.Label>
              <FormattedMessage id="scenarios-pending-regen-instructions-label" defaultMessage="Hints (optional, stored on job)" />
            </Form.Label>
            <Form.Control
              as="textarea"
              rows={2}
          ...
          <Form.Group>
            <Form.Label>
              <FormattedMessage id="scenarios-pending-regen-feedback-label" defaultMessage="What should change" />
            </Form.Label>
            <Form.Control
              as="textarea"
              rows={5}
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" disabled={bulkWorking} onClick={submitAiRegeneration}>
            {bulkWorking ? <Spinner animation="border" size="sm" /> : <FormattedMessage id="scenarios-pending-regen-submit" defaultMessage="Queue" />}
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      >
        <Modal.Header closeButton={!bulkWorking && !actionBusy}>
          <Modal.Title>
            {rejectTargetIds && rejectTargetIds.length > 1 ? (
              <FormattedMessage id="scenarios-bulk-reject-title" defaultMessage="Reject selected drafts" />
            ) : (
              <FormattedMessage id="scenarios-single-reject-title" defaultMessage="Reject draft" />
            )}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group>
            <Form.Label>
              <FormattedMessage id="scenarios-reject-reason" defaultMessage="Reason (optional)" />
            </Form.Label>
        ...
        <Modal.Footer>
          ...
          <Button variant="danger" disabled={!!bulkWorking || !!actionBusy} onClick={submitReject}>
            <FormattedMessage id="scenarios-reject-submit" defaultMessage="Reject" />
          </Button>
        </Modal.Footer>
      </Modal>
      ...
      <Modal show={bulkDiscardModal} onHide={() => !bulkWorking && setBulkDiscardModal(false)} centered>
        <Modal.Header closeButton={!bulkWorking}>
          <Modal.Title>
            <FormattedMessage id="scenarios-bulk-discard-title" defaultMessage="Discard selected drafts?" />
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p className="small text-muted mb-0">
            <FormattedMessage
              id="scenarios-bulk-discard-body"
      ...
      <Modal show={discardJobModal != null} onHide={() => !bulkWorking && setDiscardJobModal(null)} centered>
        <Modal.Header closeButton={!bulkWorking}>
          <Modal.Title>
            <FormattedMessage id="scenarios-discard-job-title" defaultMessage="Discard all pending for this job?" />
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p className="small text-muted mb-0">
            <FormattedMessage
              id="scenarios-discard-job-body"
              defaultMessage="Job {id}: every pending scenario in this batch will be discarded."
      ...
      <GenerationValidationModal
        show={validatorRowId != null && scenarioValFragment != null}
        onHide={() => setValidatorRowId(null)}
        project={validatorProjectForModal}
        artifactKind="test_scenarios"
        titleId="scenarios-gen-val-title"
        titleDefaultMessage="Validator agent — test scenario draft"
        labelId="scenarios-gen-val-intro"
        labelDefaultMessage="Rubric: correctness, requirement coverage, atomicity, scenario type alignment, step/data quality, automation readiness, and traceability. Results are advisory."
        requestFragment={scenarioValFragment}
      />
    </div>
  );
}
