...
from .recording_hooks import start_desktop_recording, stop_desktop_recording
...
class WindowsExecutor(BaseRobotExecutor):
    ...
    def _cyfast_original_before_execution(
        self,
        request: Any,
    ...
    def before_execution(self, request, workspace, output_directory, cancellation):

        context = self._cyfast_original_before_execution(request, workspace, output_directory, cancellation)

        start_desktop_recording(request, output_directory, "WINDOWS")

        return context


    def after_execution(self, request, workspace, output_directory, context, cancellation):

        try:

            return super().after_execution(request, workspace, output_directory, context, cancellation)

        finally:

            stop_desktop_recording(request)
...
class LinuxExecutor(BaseRobotExecutor):
    ...
    def before_execution(self, request, workspace, output_directory, cancellation):
        context = super().before_execution(request, workspace, output_directory, cancellation)
        start_desktop_recording(request, output_directory, "LINUX")
        return context
