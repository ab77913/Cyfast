...
def patch_general_management_index(text: str) -> str:
    ...
    if require_marker not in text:
        require_marker = 'const windowsRoutes = require("./routes/windows-routes");'
    requires = "\n".join(
        [
    ...
    if register_marker not in text:
        register_marker = 'await fastify.register(windowsRoutes, { prefix: "/" });'
    registrations = "\n".join(
        [
...
def patch_contract(text: str) -> str:
    if "CYFAST_BLOCKING_RUNTIME_CODES" not in text:
        ...
        signature = re.search(r"function\s+classifyFailure\s*\([^)]*\)\s*\{", text[position:])
        if not signature:
            raise ValueError("classifyFailure opening brace was not found")
        opening = position + signature.end() - 1
        insertion = '''
