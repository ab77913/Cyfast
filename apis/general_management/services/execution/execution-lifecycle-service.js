...
function createExecutionLifecycle(dependencies = {}) {
  ...
  async function finalizeRun(runId, result, actor) {
    ...
    if (proof.pass) {
      return store.transitionRun(runId, actor, RUN_STATES.PASSED, {
    ...
    if (BLOCKING_FAILURES.has(classification)) {
      return store.transitionRun(runId, actor, RUN_STATES.BLOCKED, terminalPatch, {
        event_type: "execution.blocked.v1",
        actor_type: "SYSTEM",
        payload: { classification },
      });
    }
    return store.transitionRun(runId, actor, RUN_STATES.FAILED, terminalPatch, {
  ...
  async function cancelRun(runId, actor) {
    ...
    return store.transitionRun(runId, actor, RUN_STATES.CANCELLED, {
      cancelled_at: now(),
      finished_at: now(),
