...
const requirementGenerationService = require("../services/requirement-generation-service");
...
const createJob = async (req, res) => {
  try {
    ...
    const job = await requirementGenerationService.createGenerationJob({
...
const listJobs = async (req, res) => {
  try {
    ...
    if (!projectId)
      return res.status(400).json({ message: "project_id is required" });
    const data = await requirementGenerationService.listJobs(
      Number(projectId),
      req.query
    );
...
const getJob = async (req, res) => {
  try {
    const job = await requirementGenerationService.getJob(
...
const regenerateJob = async (req, res) => {
  try {
    ...
    const job =
      await requirementGenerationService.regenerateGenerationJob(svc);
    return res.status(202).json({
      ...(job?.toJSON ? job.toJSON() : job),
      async_processing: true,
    });
...
const listPending = async (req, res) => {
  try {
    ...
    const payload = await requirementGenerationService.listPending(
      Number(projectId),
      page,
      size,
    );
...
const approveCandidate = async (req, res) => {
  try {
    ...
    const row = await requirementGenerationService.approveCandidate(
      id,
      userId(req)
    );
...
const rejectCandidate = async (req, res) => {
  try {
    ...
    const row = await requirementGenerationService.rejectCandidate(
      id,
      userId(req),
      reason
    );
...
const bulkApprove = async (req, res) => {
  try {
    ...
    const out = await requirementGenerationService.bulkApproveCandidates({
      projectId: Number(projectId),
...
const bulkReject = async (req, res) => {
  try {
    ...
    const out = await requirementGenerationService.bulkRejectCandidates({
      projectId: Number(projectId),
      candidateIds: Array.isArray(candidateIds) ? candidateIds : [],
      userId: userId(req),
      reason,
...
const bulkDiscard = async (req, res) => {
  try {
    ...
    const out = await requirementGenerationService.bulkDiscardCandidates({
      projectId: Number(projectId),
      candidateIds: Array.isArray(candidateIds) ? candidateIds : [],
      userId: userId(req),
      reason,
...
const regenerateSelectedCandidates = async (req, res) => {
  try {
    ...
    const job =
      await requirementGenerationService.regeneratePendingCandidatesWithFeedback(
        svc,
      );
...
const bulkRegenerateJobs = async (req, res) => {
  try {
    ...
    const out =
      await requirementGenerationService.bulkRegenerateRequirementJobs({
        projectId: Number(projectId),
...
const discardPendingJobs = async (req, res) => {
  try {
    ...
    const out = await requirementGenerationService.bulkDiscardPendingForJobs({
      projectId: Number(projectId),
      jobIds: Array.isArray(jobIds) ? jobIds : [],
      reason,
    });
