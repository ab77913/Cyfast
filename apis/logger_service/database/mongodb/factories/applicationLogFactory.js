...
const getByFilter = async (filters, sort = null, page = null, size = null) => {
  ...
  const logsData = ApplicationLog.find(conditions)
    .limit(limit)
    .skip(offset)
    .sort(sortCondition)
    .then((data) => {
  ...
  const totalItems = ApplicationLog.countDocuments(conditions).then((count) => {
    return count;
  });

  const applicationLogs = Promise.all([logsData, totalItems]).then((results) => {
...
const getCountByFilter = async (filters) => {
  const totalItems = ApplicationLog.countDocuments(filters).then((count) => {
    return count;
  });
...
const getLogById = async (id) => {
  const applicationLog = ApplicationLog.findById(id).then((data) => {
    return data;
  });
...
const createLog = async (data) => {
  const applicationLog = new ApplicationLog(data);

  const response = applicationLog.save(applicationLog).then((data) => {
    return data;
  });
