...
const getExecutionDuration = async (projectId) => {
  try {
    let pool = await sql.connect(dbConfig);
    const sqlQueries = await utils.loadSqlQueries("orchestrations");
    const event = await pool.request().input("projectId", sql.Int, projectId).query(sqlQueries.totalExecutionDuration);
...
const getLatestExecutionsByProjectId = async (projectId) => {
  try {
    let pool = await sql.connect(dbConfig);
    const sqlQueries = await utils.loadSqlQueries("orchestrations");
    const event = await pool.request().input("projectId", sql.Int, projectId).query(sqlQueries.latestExecutionsByProjectId);
    return event.recordset;
  } catch (error) {
    return error.message;
  }
